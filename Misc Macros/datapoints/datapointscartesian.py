...
def square_points(centre, radius, resolution):
    '''
    Plots a grid of points within a circle radius R around a centre position with resolution defined

    :param centre:          List or tuple of centre coordinates. Eg [2000, 3000]
    :param radius:          The radius of the ring
    :param resolution:      The distance between points in our grid (preferably divisible by the radius)
    :return positions:      List of [x,y] coordinates on ring.

    '''
    # ZERO HERE DEFINES THE 'CENTRE' POINT FOR OUR VALUES

    # Determine how many points you can get along X and Y
    pos_list = []
    rang = int(radius*2/(resolution))
    # Start at bottom of y axis
    position = [0, 0-radius]
    # i defines how far up the y axis you've gone
    for i in range(rang+1):
        while(np.sqrt(position[0]**2 + position[1]**2) <= radius):

            # if on y axis, append once, if not, do the flipside too.
            if (position[0] == 0):
            ...
            # Move
            position = [position[0]+resolution, position[1]]

        # update y axis
        position = [0, position[1]+resolution]

    # Move all positions by a set amount, to the centre
    pos_list1 = [(pos_list[i][0] + centre[0]) for i in range(len(pos_list))]
    pos_list2 = [(pos_list[i][1] + centre[1]) for i in range(len(pos_list))]
    pos_list = [[pos_list1[i],pos_list2[i]] for i in range(len(pos_list))]
    return pos_list
...
position_values = square_points([4946,5595], 1900, 190)
